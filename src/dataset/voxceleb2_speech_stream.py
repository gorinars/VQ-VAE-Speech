from dataset.voxceleb2_dataset import Voxceleb2Dataset
from dataset.voxceleb2 import Voxceleb2
...
class Voxceleb2SpeechStream(object):

    def __init__(self, configuration, gpu_ids, use_cuda):
        voxceleb2 = Voxceleb2(configuration['data_root'], ratio=configuration['train_val_split'])
        self._training_data = Voxceleb2Dataset(voxceleb2.audios_train, voxceleb2.speaker_dic, voxceleb2.utterences, configuration)
        self._validation_data = Voxceleb2Dataset(voxceleb2.audios_val, voxceleb2.speaker_dic, voxceleb2.utterences, configuration)
        ...
        self._speaker_dic = voxceleb2.speaker_dic
    ...
    def export_to_features(self, voxceleb2_path, configuration):
        if not os.path.isdir(voxceleb2_path):
            raise ValueError("Voxceleb2 dataset not found at path '{}'".format(voxceleb2_path))

        # Create the features path directory if it doesn't exist
        features_path = voxceleb2_path + os.sep + configuration['features_path']
