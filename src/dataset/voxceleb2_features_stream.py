from dataset.voxceleb2_features_dataset import Voxceleb2FeaturesDataset
...
class Voxceleb2FeaturesStream(object):

    def __init__(self, voxceleb2_path, configuration, gpu_ids, use_cuda):
        self._normalizer = None
        if configuration['normalize']:
            with open(configuration['normalizer_path'], 'rb') as file:
                self._normalizer = pickle.load(file)

        self._training_data = Voxceleb2FeaturesDataset(voxceleb2_path, 'train', self._normalizer, features_path=configuration['features_path'])
        self._validation_data = Voxceleb2FeaturesDataset(voxceleb2_path, 'val', self._normalizer, features_path=configuration['features_path'])
        ...
        self._speaker_dic = self._make_speaker_dic(voxceleb2_path + os.sep + 'raw' + os.sep + 'Voxceleb2-Corpus')
        self.voxceleb2_path = voxceleb2_path
